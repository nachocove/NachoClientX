...
namespace NachoClient.AndroidClient
{
    public class HotSummaryFragment : Fragment
    {
        ...
        void DeadlinesView_Click (object sender, EventArgs e)
        {
            ...
            var intent = DeadlineActivity.ShowDeadlineFolderIntent (Activity, folder);
        ...
        void DeferredView_Click (object sender, EventArgs e)
        {
            ...
            var intent = DeferredActivity.ShowDeferredFolderIntent (Activity, folder);
            StartActivity (intent); 
        }
        ...
        public void Update (McAccount account)
        {
            NcTask.Run (() => {
                ...
                InvokeOnUIThread.Instance.Invoke (() => {
                    inboxMessageCountView.Text = String.Format ("Go to Inbox ({0:N0} unread)", unreadMessageCount);
                    deferredMessageCountView.Text = String.Format ("Go to Deferred Messages ({0:N0})", deferredMessageCount);
                    deadlinesMessageCountView.Text = String.Format ("Go to Deadlines ({0:N0})", deadlineMessageCount);
                    // FIMXE LTR.
                });
