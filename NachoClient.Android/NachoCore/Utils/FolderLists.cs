    ...
    public class FolderLists
    {
        ...
        public void Create (int accountId, bool hideFakeFolders)
        {
            ...
            foreach (var account in McAccount.GetAllConfiguredNonDeviceAccounts()) {
                ...
                CreateAccountList (account.Id, out defaultList, out rootList);
                var rootListNode = new Node ();
                rootListNode.children = rootList;
                var defaultListNode = new Node ();
                defaultListNode.children = defaultList;
                var accountNode = new Node ();
                accountNode.account = account;
                accountNode.children.Add (rootListNode);
                accountNode.children.Add (defaultListNode);
                displayList.Add (new DisplayElement (accountNode, 0));
            }
            MarkLastInSection ();
        }
