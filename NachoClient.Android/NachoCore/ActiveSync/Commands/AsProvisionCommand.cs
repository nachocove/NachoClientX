...
namespace NachoCore.ActiveSync
{
    public class AsProvisionCommand : AsCommand
    {
        ...
        // FIXME - need to handle AsEvt events.
        ...
        public override XDocument ToXDocument (AsHttpOperation Sender)
        {
            var policy = new XElement (m_ns + Xml.Provision.Policy, 
                new XElement (m_ns + Xml.Provision.PolicyType, "MS-EAS-Provisioning-WBXML"),
                new XElement (m_ns + Xml.Provision.PolicyKey, DataSource.ProtocolState.AsPolicyKey));
            if (AckOp == Sender) {
                // FIXME - need to reflect actual status here.
                policy.Add (new XElement (m_ns + Xml.Provision.Status, "1"));
            }
            var doc = AsCommand.ToEmptyXDocument ();
            doc.Add (new XElement (m_ns + Xml.Provision.Ns, new XElement (m_ns + Xml.Provision.Policies, policy)));
            return doc;
        }

        public override Event ProcessResponse (AsHttpOperation Sender, HttpResponseMessage response, XDocument doc)
        {
            switch ((Xml.Provision.ProvisionStatusCode)Convert.ToUInt32 (doc.Root.Element (m_ns + Xml.Provision.Status).Value)) {
            case Xml.Provision.ProvisionStatusCode.Success:
                NcProtocolState update = DataSource.ProtocolState;
                update.AsPolicyKey = doc.Root.Element (m_ns + Xml.Provision.Policies).Element (m_ns + Xml.Provision.Policy).Element (m_ns + Xml.Provision.PolicyKey).Value;
